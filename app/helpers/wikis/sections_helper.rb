module Wikis::SectionsHelper
  ...
  def wiki_body_html(wiki = @wiki)
    ...
    decorate_wiki_sections(body_html, wiki)
  end
  ...
  def wrap_in_div(doc, section, is_full_wiki)
    ...
    # don't really think below is best way, but works for now:
    end_before = find_heading_node(doc, @wiki.successor_for_section(section).try.name) if is_full_wiki
  ...
  def decorate_wiki_sections(html, section)
    ...
    doc.search('h4 a.anchor, h3 a.anchor, h2 a.anchor, h1 a.anchor').each do |anchor|
      ...
      #commented out for now. it works, but probably we want next in some cases
      #when would the section/wiki not include the subsection?
      #next unless section.all_sections.include? subsection
      add_edit_link_to_heading(anchor, subsection)
      wrap_in_div(doc, subsection, section == @wiki)
    end
    doc.to_html
  end
