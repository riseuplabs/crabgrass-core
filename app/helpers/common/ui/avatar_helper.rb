module Common::Ui::AvatarHelper
  ...
  def avatar_url_for(entity, size = 'medium')
    if entity
      format('/avatars/%s/%s.jpg?%s', entity.avatar_id || 0, size, entity.respond_to?(:version) ? entity.version : entity.updated_at.to_i)
    else
      format('/avatars/0/%s.jpg', size)
    end
  end

  def avatar_style(entity, size = 'medium')
    format('background-image: url(%s)', avatar_url_for(entity, size))
  end

  def square_avatar_style(entity, size = 'medium')
    format('background-image: url(%s); width: %spx; height: %spx;', avatar_url_for(entity, size), avatar_height(size), avatar_height(size))
  end
  ...
  def upload_avatar_link(entity)
    link_to_modal :upload_image.t, edit_avatar_path(entity),
                  icon: 'picture_edit',
                  class: 'inline'
  end
