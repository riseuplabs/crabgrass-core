module UserRecords
  def hidden_user
    records[:hidden_user] ||= FactoryBot.create(:user).tap do |hide|
  ...
  def blocking_user
    records[:blocking_user] ||= FactoryBot.create(:user).tap do |blocking|
  ...
  def public_user
    records[:public_user] ||= FactoryBot.create(:user).tap do |pub|
      pub.grant_access! public: :view
    end
  end

  def user
    records[:user] ||= @user ||= FactoryBot.create(:user)
  end
  ...
  def other_user
    records[:other_user] ||= FactoryBot.create :user
  end
  ...
  def peer_of(other)
    FactoryBot.create(:user).tap do |peer|
