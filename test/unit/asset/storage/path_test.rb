...
class Asset::Storage::PathTest < ActiveSupport::TestCase
  ...
  def test_private_filename
    assert_equal "%s/0000/%04d/image.png" % [ASSET_PRIVATE_STORAGE, asset_id],
      path.private_filename
  end

  def test_private_thumbnail_filename
    assert_equal "%s/0000/%04d/image_small.png" % [ASSET_PRIVATE_STORAGE, asset_id],
      path.private_filename(thumbnail_filename)
  end

  def test_public_filename
    assert_equal "%s/%s/image.png" % [ASSET_PUBLIC_STORAGE,asset_id],
      path.public_filename
  end

  def test_public_thumbnail_filename
    assert_equal "%s/%s/image_small.png" % [ASSET_PUBLIC_STORAGE,asset_id],
      path.public_filename(thumbnail_filename)
  end

  def test_url
    assert_equal "/assets/%s/image.png" % asset_id, path.url
  end

  def test_thumbnail_url
    assert_equal "/assets/%s/image_small.png" % asset_id,
  ...
  def test_versioned_url
    assert_equal "/assets/%s/versions/3/image_small.png" % asset_id,
  ...
  def asset_id; 123; end
  def filename; 'image.png'; end
  def thumbnail_filename; 'image_small.png'; end
end
