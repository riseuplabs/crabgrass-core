...
class ActivityTest < ActiveSupport::TestCase
  ...
  def test_contact
    ...
    act = FriendActivity.for_all(u1).find(:first)
  ...
  def test_user_destroyed
    ...
    act = UserDestroyedActivity.for_all(u1).find(:first)
  ...
  def test_membership
    ...
    assert_nil UserJoinedGroupActivity.for_all(notified_user).find_by_subject_id(user.id), 'the notified user does not get this kind of activity message'
    ...
    act = UserJoinedGroupActivity.for_all(user).last
    assert_equal group.id, act.group.id
    ...
    act = GroupLostUserActivity.for_all(notified_user).last
    assert_activity_for_user_group(act, user, group)

    act = GroupLostUserActivity.for_group(group, notified_user).last
    assert_activity_for_user_group(act, user, group)

    act = UserLeftGroupActivity.for_all(user).last
    assert_activity_for_user_group(act, user, group)
  end
