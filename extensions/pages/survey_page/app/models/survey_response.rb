...
class SurveyResponse < ApplicationRecord
  ...
  def answers_attributes=(attrs)
    ...
    attrs.each do |_key, params|
      ...
      answers << question.build_answer(params)
    end
  end

  def validate_associated_records_for_answers
    answers.each do |answer|
      label = format("'%s'", truncate(answer.question.label))
      unless answer.valid?
        answer.errors.each { |_attr, msg| errors.add(label, msg) }
      end
    end
  end
