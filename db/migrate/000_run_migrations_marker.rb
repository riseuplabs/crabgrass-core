class RunMigrationsMarker < ActiveRecord::Migration
  ...
  def old_migration_number
    @old_migration_number ||= begin
                                ActiveRecord::Base.connection.select_one("SELECT version FROM #{old_schema_info_table_name}")['version'].to_i
                              rescue
                                0
                              end
  end
  ...
  def migration_numbers
    Dir["#{Rails.root}/db/migrate/[0-9]*_*.rb"].collect { |file| file[/(\d+)_[^\/]+\.rb$/][Regexp.last_match(1)].to_i }.sort
  end
